...
class SubmitResults(commands.Cog):
    ...
    @commands.command(
        help='type !submit <winner username> <winner score> <loser score> <loser username> <"Game Mode Name" (in quotes)>)'
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def submit(
        self, ctx: commands.Context, user1: str, score1: int, score2: int, user2: str, game_mode_name: str
    ):
        ...
        try:
            ...
            dm_embed.add_field(
                ...
                value=(
                    "A new manual submission has been created in the mario baseball server.\n"
        ...
        def check_confirm(rxn, usr):
            return usr.id == APPROVER_ID and rxn.emoji in (CHECK_EMOJI, X_EMOJI) and rxn.message.id == ctx.message.id

        try:
            reaction, _ = await self.client.wait_for("reaction_add", timeout=SUBMISSION_TIMEOUT, check=check_confirm)
        ...
        async with self.client.session.post(MANUAL_SUBMIT_ENDPOINT, json=manual_submit) as response:
            if response.status >= 400:
                ...
                fail_embed.add_field(
                    name="Error", value=f"The Project Rio API rejected the submission (status {response.status})."
                )
    ...
    @staticmethod
    def _result_embed(
        title: str, color: int, winner: str, winner_score: int, loser_score: int, loser: str, game_mode: str
    ) -> discord.Embed:
